...
class branding(minqlx.Plugin):
    ...
    def brand_map(self):
        if self.get_cvar("qlx_serverBrandName") is None:
            self.set_cvar("qlx_serverBrandName", self.game.map_title)

        if self.get_cvar("qlx_brandingPrependMapName", bool):
            top_branding = self.game.map_title + " " + self.get_cvar("qlx_serverBrandName")
        else:
            top_branding = self.get_cvar("qlx_serverBrandName")

        if self.get_cvar("qlx_brandingAppendGameType", bool):
            minqlx.set_configstring(3, top_branding + " " + self.game.type)
        else:
            minqlx.set_configstring(3, top_branding)

        if self.get_cvar("qlx_serverBrandTopField") is not None:
            cs = self.game.map_subtitle1
            if cs:
                cs += " - "
            minqlx.set_configstring(678, cs + self.get_cvar("qlx_serverBrandTopField"))

        if self.get_cvar("qlx_serverBrandBottomField") is not None:
        ...
        if self.get_cvar("qlx_rainbowBrandName", bool):
            ...
            rainbow = rotating_colors()
            res = ""
            for i in range(len(map_name)):
                res += "^{}{}".format(next(rainbow), map_name[i])

            minqlx.set_configstring(3, res)

    def player_connect(self, player):
        if self.get_cvar("qlx_connectMessage") is not None:
            if player not in self.playerConnectedYetList:
                ...
                return "{}\n^7This server is running ^4branding.py^7. ^2http://github.com/tjone270/Quake-Live^7.\n".format(
                    self.get_cvar("qlx_connectMessage")
                )

        return None

    def player_loaded(self, player):
        if self.get_cvar("qlx_loadedMessage") is not None:
    ...
    def game_countdown(self):
        if self.get_cvar("qlx_countdownMessage") is not None:
            self.center_print(self.get_cvar("qlx_countdownMessage"))

    def game_end(self, data):
        if self.get_cvar("qlx_endOfGameMessage") is not None:
