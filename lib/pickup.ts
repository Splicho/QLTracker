...
export async function fetchPickupPublicState() {
  ...
  if (!response.ok) {
    throw new PickupApiError(response.status, "Pickup realtime state is unavailable.");
  }
...
export async function fetchPickupPlayerState(sessionToken: string) {
  ...
  if (!response.ok) {
    throw new PickupApiError(response.status, "Pickup player state is unavailable.");
  }
