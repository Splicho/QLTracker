...
import { appendPickupLog, summarizePickupPlayerState, summarizePickupPublicState } from "@/lib/pickup-log";
import {
  applyMockReadyUp,
  applyMockVetoBan,
...
type PickupStateAction =
  ...
  | { type: "dismiss_completed_match"; idleState: PickupPlayerState; matchId: string }
  | { type: "reset_session" }
    ...
    case "apply_player_state": {
      ...
      const viewerReady =
        nextState.stage === "ready_check"
          ? [...nextState.match.teams.left, ...nextState.match.teams.right].some(
              (player) =>
                player.id === nextState.viewer.id && player.readyState === "ready",
            )
          : false;
      ...
      return {
        ...
        dismissedCompletedMatchId:
          nextState.stage === "completed" ? state.dismissedCompletedMatchId : null,
...
export function usePickupState(
  ...
  const latestPlayerStateRef = useRef<PickupPlayerState | null>(initialPlayerState);
  const {
    mockStage,
    playerState,
    publicState,
  ...
  const logPublicStateSnapshot = useCallback((source: string, nextState: PickupPublicState) => {
  ...
  const logPlayerStateSnapshot = useCallback((source: string, nextState: PickupPlayerState) => {
  ...
  const playerQuery = useQuery({
    ...
    enabled: enabled && realtimeAvailable && sessionToken.trim().length > 0 && !mockMode,
    initialData:
      initialPlayerState && sessionToken.trim().length > 0
        ? initialPlayerState
        : undefined,
    retry: (failureCount, error) => {
      if (error instanceof PickupApiError && (error.status === 401 || error.status === 403)) {
        return false;
      }
  ...
  const applyNextPlayerState = useCallback(
    (nextState: PickupPlayerState) => {
      dispatch({
  ...
  useEffect(() => {
    ...
    const players = [...playerState.match.teams.left, ...playerState.match.teams.right];
    const viewerReady = players.some(
      (player) => player.id === playerState.viewer.id && player.readyState === "ready",
    );
  ...
  }, [applyNextPlayerState, logPlayerStateSnapshot, playerQuery.data, realtimeAvailable]);
  ...
  }, [applyNextPlayerState, enabled, mockMode, realtimeAvailable, sessionToken]);
  ...
  useEffect(() => {
    ...
    const refreshAuthoritativeState = async () => {
      const [nextPublicResult, nextPlayerResult] = await Promise.all([
        publicQuery.refetch(),
        sessionToken.trim().length > 0 ? playerQuery.refetch() : Promise.resolve(null),
      ]);
      ...
      if (nextPublicResult?.data) {
        ...
        logPublicStateSnapshot("pickup.ready_timeout.public_state", nextPublicResult.data);
      }

      if (nextPlayerResult?.data) {
        applyNextPlayerState(nextPlayerResult.data);
        logPlayerStateSnapshot("pickup.ready_timeout.player_state", nextPlayerResult.data);
      }
    };
  ...
  return {
    ...
    stateLoading: mockMode ? false : publicQuery.isLoading || playerQuery.isLoading,
