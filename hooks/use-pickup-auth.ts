...
export function usePickupAuth(initialState?: InitialPickupAuthState) {
  ...
  useEffect(() => {
    ...
    const hasNextSession = window.localStorage.getItem(PICKUP_SESSION_STORAGE_KEY);
    const hasNextGuestMode = window.localStorage.getItem(PICKUP_GUEST_STORAGE_KEY);
