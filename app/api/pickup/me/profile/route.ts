...
export async function PATCH(request: Request) {
  try {
    ...
    const updatedPlayer = await updatePickupPlayerProfileMedia(session.player.id, {
      countryCode: parseOptionalCountryCode(payload.countryCode),
      customAvatarUrl: parseOptionalImageUrl(payload.customAvatarUrl, "Avatar"),
    ...
    if (staleImageUrls.length > 0) {
      void deletePickupImagesFromR2(staleImageUrls).catch((error) => {
        console.error("Failed to remove old pickup profile images from R2:", error);
      });
    }
