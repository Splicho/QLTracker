...
export function RootChrome({
  ...
  const activePickupViewer = pickupState.playerState?.viewer ?? pickupAuth.player;
  const activeQueueId =
  ...
  const activePickupQueue = activeQueueId
    ? (pickupState.publicState?.queues.find((queue) => queue.id === activeQueueId) ??
      pickupState.publicState?.queue ??
      null)
    : null;
  const pickupStackPlayers =
